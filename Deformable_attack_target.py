...
class Deformable_ink_attack_target:
    ...
    def CaculateFitness(self, DE_all, image, true_name, target_name, mask):
        attack_images = []
        ...
        target_names = [target_name] * len(attack_images)
        with torch.no_grad():
            ...
            target_scores = utils.get_label_score(self.gallery_features, perturbed_features, self.gallery_names, target_names)
        for DE_one, top1_label, top1_score, top2_label, top2_score in zip(DE_all, top1_labels, top1_scores, target_names, target_scores):
            if top1_label == target_name:
                DE_one.fitness = 0
            else:
                DE_one.fitness = top1_score - top2_score
        return DE_all, top1_labels
        
    def attack(self, DE_all, image, true_name, target_name, mask):
        t = 0
        while(t < self.maxiter):
            ...
            DE_next, top1_labels_next = self.CaculateFitness(DE_next, image, true_name, target_name, mask)
    ...
    def excute(self, args, images):
        check = 0
        for image, true_name, target_name in zip(images, self.true_names, self.target_names):
            print(f'solve targeted attack:{true_name}->{target_name}')
            image = torch2uint8gray(image)
            mask = self.generate_face_mask(image)
            DE_all = self.initialize(mask)
            DE_all, top1_labels, attack_succ = self.attack(DE_all, image, true_name, target_name, mask)
            if attack_succ:
                print('attack_succ! BUT DEF!')
                first_different_label_index = top1_labels.index(target_name)
                output_path = f'DEF-attack-{true_name}-{target_name}-anchor{self.AnchorNum}-area{self.AreaNum}.jpg'
                output_dir = f'{args.model}-{args.data_type}-{args.attack_type}-images'
