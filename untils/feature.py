...
def face_landmarks(initial_pic):
    ...
    pic_array = cv2.cvtColor(pic_array, cv2.COLOR_GRAY2RGB)
...
def circle_mark(facemask, dot, brw):
    ...
    for i in range(dotlen):
        x1, y1 = dot[i]
        if 0 <= x1 < facemask.shape[0] and 0 <= y1 < facemask.shape[1]:
            facemask[x1, y1] = brw
        ...
        if y2 - y1 != 0:
            ...
            for t in range(symbol * (y2 - y1) - 1):
                ...
                if 0 <= x3 < facemask.shape[0] and 0 <= y3 < facemask.shape[1]:
                    facemask[x3, y3] = brw
    ...
    for h in range(lower, upper + 1):
        ...
        for i in range(cruitl - 1, cruitr + 2):
            if 0 <= i < facemask.shape[0] and 0 <= h < facemask.shape[1]:  # 添加边界检查
                if facemask[i][h] == brw:
                    left = i
                    break

        for j in reversed(range(cruitl - 1, cruitr + 2)):
            if 0 <= j < facemask.shape[0] and 0 <= h < facemask.shape[1]:  # 添加边界检查
                if facemask[j][h] == brw:
                    right = j
                    break
        ...
        if left_cursor != right_cursor:
            while True:
                if left_cursor >= facemask.shape[0]: break  # 添加边界检查
                facemask[left_cursor][h] = brw
                left_cursor += 1
                if facemask[left_cursor][h] == brw:
                    break
            while True:
                if right_cursor < 0: break  # 添加边界检查
